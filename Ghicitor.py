...
def initializareFrecv():
    global frecvCuvinte
    # frecvCuvinte este un dictionar cu toate cuvintele, ce are ca valori dictionare cu toate cele 3^5 posibile feedback-uri
    # si de cate ori apare fiecare feedback pentru un anumit cuvant, raportat la toate celelalte cuvinte din lista
    frecvCuvinte = {}
    global cuvinte
    n = len(cuvinte)
    for i in range(n):
        ...
        frecvCuvinte[cuvinte[i]] = frecv

        # calculam frecventa posibilelor feedback-uri
        for j in range(n):
            rel = ""
            for k in range(5):
                ...
                for j in range(3):
                    for k in range(3):
                        for l in range(3):
                            for m in range(3):
                                for o in range(3):
                                    p = frecvCuvinte[cuvinte[i]][str(j) + str(k) + str(l) + str(m) + str(o)] / n
                                    if p != 0:
                                        entropieCuv += p * math.log2(1 / p)
                                        entropieCuv = round(entropieCuv, 10)
